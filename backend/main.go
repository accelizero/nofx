...
func main() {
	...
	default:
		...
		for i := 0; i < healthCheckRetries; i++ {
			...
			resp, err := http.Get(fmt.Sprintf("http://localhost:%d/health", cfg.APIServerPort))
			if err == nil {
				resp.Body.Close()
				healthCheckSuccess = true
				log.Printf("✓ API服务器已就绪（第 %d 次检查成功）", i+1)
				break
			}
		...
		if !healthCheckSuccess {
			...
			case err := <-apiErrChan:
				log.Fatalf("❌ API服务器启动失败: %v", err)
			default:
				log.Printf("⚠️  API服务器健康检查失败，但未检测到启动错误，继续运行（服务器可能仍在启动中）")
			}
		}
	}
	...
	if err := apiServer.Shutdown(ctx); err != nil {
		log.Printf("⚠️  API服务器关闭时出错: %v", err)
	} else {
